...
class PatchD
{
    public static void Prefix()
    {
        ...
        r = new Rect(Screen.width/2 - dw/2 - 2, 20 - 2, dw + 4, 40 + 4);
        c = new Color(0, 0, 0, 1.0F);
        TestEffectPack.Base.GUIDrawRect(r, c);

        int hw = (int)((dw * h) / tot);


        r = new Rect(Screen.width / 2 - dw / 2, 20, hw, 40);
        c = new Color(1.0F, 0, 0, 1.0F);
        TestEffectPack.Base.GUIDrawRect(r, c);
...
namespace TestEffectPack
{
    ...
    public class Base
    {
        ...
        public static bool isPlane()
        {
            try
            {
                if (!SceneLoader.SceneName.StartsWith("scene_level_flying")) return false;
                return true;
            }
            catch (Exception e)
            {
        ...
        private static Texture2D _staticRectTexture = null;
